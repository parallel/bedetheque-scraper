...
export class Scraper {
  static async getSeriesUrlFromLetter(
    letter: string,
    proxy: Proxy = null
  ): Promise<string[]> {
    const $ = await Utils.requestWithProxy(
      `https://www.bedetheque.com/bandes_dessinees_${letter}.html`,
      proxy
    );
  ...
  static async getAuthorsUrlFromLetter(
    letter: string,
    proxy: Proxy = null
  ): Promise<string[]> {
    const $ = await Utils.requestWithProxy(
      `https://www.bedetheque.com/liste_auteurs_BD_${letter}.html`,
      proxy
    );
